"""Animated motion-graphics scene builder — 100% FREE.

Builds a self-playing animated HTML "scene player" using Iconify vector
illustrations, CSS keyframe motion, caption boxes and pill labels.
Adapted from LMSarresto — import paths updated; logic unchanged.

Styles: "modern" | "flatcolor" | "whiteboard"
"""
...
from modules.video.generators.assets import get_illustration

_EMOJI = {
    "warning": "⚠️", "fire": "🔥", "car": "🚗", "road": "🛣️", "bike": "🚲",
    "bicycle": "🚲", "person": "🚶", "eye": "👁️", "speed": "🚦", "sleep": "😴",
    "phone": "📱", "shield": "🛡️", "hardhat": "👷", "check": "✅", "helmet": "🪖",
    "traffic": "🚦", "alert": "🚨", "book": "📘", "star": "⭐", "heart": "❤️",
    "medical": "🏥", "firstaid": "🩹", "fire_extinguisher": "🧯",
}
...
_ICON_QUERY = {
    ...
    "heart": "heart", "medical": "medical", "firstaid": "first aid",
}

_STOP = {
    "the", "a", "an", "and", "or", "for", "of", "to", "in", "on", "your",
    "safety", "basics", "common", "understanding", "introduction", "tips",
    "everyone", "practices", "matters", "why", "what", "how", "lesson",
    "ka", "ki", "ke", "hai", "hain", "aur", "se", "par",  # common Hindi stop words
}
...
def build_scenes(
    ...
    """Turn lesson content into a timed sequence of illustrated animated scenes."""
    scenes: list[dict] = []
    topic = _keyword(lesson_title)
    ...
    # 3. One caption scene per important slide
    used = 0
    for s in slides:
    ...
    # 4. Warning scene (safety content)
    for s in slides:
