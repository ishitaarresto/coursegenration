"""Reveal.js HTML slide renderer.

Converts lesson data into an interactive Reveal.js presentation.
No LLM call needed — builds from existing lesson JSON fields.
...
def render_reveal_html(lesson_title: str, slides: list[schemas.SlideSpec]) -> str:
    """Render a Reveal.js interactive presentation from a list of SlideSpec objects."""
...
def lesson_to_slides(lesson: dict) -> list[schemas.SlideSpec]:
    """Build a rich list of SlideSpec objects from a lesson dict (no LLM call)."""
    title = lesson.get("lesson_title", "Lesson")
    explanation = lesson.get("simplified_explanation", "")
    takeaways = lesson.get("key_takeaways", [])
    examples = lesson.get("real_world_examples", [])
    raw_scenarios = lesson.get("safety_scenarios", [])

    slides: list[schemas.SlideSpec] = []

    # 1. Title
    slides.append(schemas.SlideSpec(type="title", heading=title, icon="book"))

    # 2. Hook — simplified explanation
    if explanation:
        slides.append(schemas.SlideSpec(type="hook", heading=title, note=explanation))

    # 3. Key takeaways as content
    if takeaways:
        slides.append(schemas.SlideSpec(
            type="content", heading="Key Points",
            bullets=[str(t) for t in takeaways[:5]], icon="checklist",
        ))

    # 4. Safety scenarios as warning slides
    for s in raw_scenarios[:2]:
        if isinstance(s, dict):
            situation = s.get("situation", "")
            correct = s.get("correct_action", "")
        else:
            situation = str(s)
            correct = ""
        bullets = []
        if situation:
            bullets.append(f"⚠️ {situation}")
        if correct:
            bullets.append(f"✅ {correct}")
        if bullets:
            slides.append(schemas.SlideSpec(
                type="warning", heading="Safety Alert", bullets=bullets, icon="warning",
            ))

    # 5. Real-world examples as timeline
    if examples:
        slides.append(schemas.SlideSpec(
            type="timeline", heading="Real-World Examples",
            bullets=[str(e) for e in examples[:4]], icon="lightbulb",
        ))
    ...
    if takeaways:
        slides.append(schemas.SlideSpec(
            ...
            bullets=[str(t) for t in takeaways[:5]], icon="star",
        ))
