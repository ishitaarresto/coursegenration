...
def _sync_generate_from_text(
    job: _CourseJob, api_key: str, vector_store: Any, embedder: Any,
    content_text: str, mode: str = "detailed",
) -> None:
    import sys, io as _io
    _captured   = _io.StringIO()
    _old_stdout = sys.stdout
    ...
    try:
        ...
        try:
            from api.course_library import library as _lib
            _lib.save(
                script_id=job.job_id, source_file=job.source_file,
                course_title=script.course_title, target_audience="learners",
...
async def generate_from_text_in_background(
    job: _CourseJob, api_key: str, vector_store: Any, embedder: Any,
    content_text: str, mode: str = "detailed",
) -> None:
    await asyncio.to_thread(
        _sync_generate_from_text, job, api_key, vector_store, embedder, content_text, mode,
    )


def _sync_generate_micro_from_text(
    job: _CourseJob, api_key: str, vector_store: Any, embedder: Any, content_text: str,
) -> None:
    import sys, io as _io
    _captured   = _io.StringIO()
    _old_stdout = sys.stdout
...
async def generate_micro_from_text_in_background(
    job: _CourseJob, api_key: str, vector_store: Any, embedder: Any, content_text: str,
) -> None:
    await asyncio.to_thread(
        _sync_generate_micro_from_text, job, api_key, vector_store, embedder, content_text,
    )
