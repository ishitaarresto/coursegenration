...
# The creative brief is the user's full reference prompt #3 (verbatim). The bridge
# below tells the model how to express that vision in the renderer's structured schema.
...
def _split_sentences(text: str) -> list[str]:
    """Split text into sentences across English + Indic punctuation."""
    import re
...
def _fallback_plan(script: str, lesson_title: str) -> WhiteboardPlan:
    ...
    # Aim for 3-6 scenes; distribute sentences as evenly as possible.
    ...
    for i in range(0, len(sentences), per):
        ...
        idx = len(scenes)
        # Title = first 4 words of the chunk's first sentence
        ...
        # Ensure the 3-element minimum the schema requires.
    ...
    # Schema needs at least 3 scenes — pad by splitting if we came up short.
    while len(scenes) < 3:
        idx = len(scenes)
        scenes.append(WBScene(
...
def generate_whiteboard_plan(
    ...
    Defense in depth so the video render can never crash on the planning step:
      1. Ask the LLM (retry once on empty/invalid output).
      2. If the LLM keeps failing, build a valid plan mechanically from the script.
    """
    instruction = (
    ...
    for _attempt in range(2):
        try:
            ...
            pass  # fall through to retry, then to the deterministic fallback

    # LLM failed twice — build a guaranteed-valid plan so the render still succeeds.
...
def generate_questions(
    ...
    try:
        ...
        # Sanitise: drop malformed MCQs, clamp indices
        clean: list[Question] = []
